...
const AdminSidebar = ({ sidebarCollapsed, mobileSidebarOpen, isMobile, onOverlayClick, user }) => {
  ...
  const userRole = (user?.role || storedUser.role || localStorage.getItem('role') || sessionStorage.getItem('role') || '').toLowerCase();
  const isAdmin = userRole === 'admin';

  const adminNavItems = [
    { path: '/', label: 'Dashboard', icon: 'M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z' },
    { path: '/attendance', label: 'Attendance & Leaves', icon: 'M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM9 10H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm-8 4H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2z' },
    ...
    { path: '/notifications', label: 'Notifications', icon: 'M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z' },
    ...
    { path: '/ai-chat', label: 'AI Assistant', icon: 'M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z' },
    { path: '/workhours', label: 'Work Hours', icon: 'M11.99 2C6.47 2 2 6.48 2 12s4.48 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67z' },
  ...
  const employeeNavItems = [
    ...
    { path: '/chat', label: 'Messages', icon: 'M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z' },
    { path: '/notifications', label: 'Notifications', icon: 'M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z' },
    { path: '/shifts', label: 'My Shifts', icon: 'M7 11h2v2H7v-2zm0-4h2v2H7V7zm4 4h2v2h-2v-2zm0-4h2v2h-2V7zm4 4h2v2h-2v-2zm0-4h2v2h-2V7zm-8 8h2v2H7v-2zm4 0h2v2h-2v-2zm4 0h2v2h-2v-2zM4 3h16c1.1 0 2 .9 2 2v14c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V5c0-1.1.9-2 2-2zm0 2v14h16V5H4z' },
    { path: '/salary', label: 'My Salary', icon: 'M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.04c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z' },
    { path: '/reports', label: 'Reports', icon: 'M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z' },
    { path: '/projects', label: 'Projects', icon: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z' },
    { path: '/holidays', label: 'Holidays', icon: 'M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM9 10H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2zm-8 4H7v2h2v-2zm4 0h-2v2h2v-2zm4 0h-2v2h2v-2z' },
    { path: '/settings', label: 'Settings', icon: 'M19.14,12.94c0.04,-0.31 0.06,-0.63 0.06,-0.94l0,-0.02c1.06,-0.81 1.87,-1.86 2.33,-3.05l-2.03,-1.58c-0.46,0.74 -1.13,1.33 -1.92,1.7l-0.02,0.01c-0.54,-0.92 -1.28,-1.71 -2.17,-2.3l0.02,-0.02c0.09,-0.84 0.01,-1.7 -0.26,-2.53l-2.39,0.96c0.21,0.76 0.26,1.57 0.13,2.36l-0.01,-0.04c-0.96,-0.18 -1.96,-0.18 -2.92,0l-0.01,-0.04c-0.13,-0.79 -0.08,-1.6 0.13,-2.36l2.39,0.96c0.27,-0.83 0.35,-1.69 0.26,-2.53l-0.02,-0.02c0.89,-0.59 1.63,-1.38 2.17,-2.3l-0.02,-0.01c-0.79,-0.37 1.46,-0.96 1.92,-1.7l2.03,-1.58c-0.46,-1.19 -1.27,-2.24 -2.33,-3.05l0,-0.02zM12,15.6c-1.98,0 -3.6,-1.62 -3.6,-3.6s1.62,-3.6 3.6,-3.6s3.6,1.62 3.6,3.6s-1.62,3.6 -3.6,3.6z' },
  ];
